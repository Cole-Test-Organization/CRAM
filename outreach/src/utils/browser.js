...
export async function loginToLinkedIn() {
    ...
    logger.info(
        "Please log in to LinkedIn in the browser window — waiting for login (auto-closes on success)",
    );
...
export async function getLinkedInPage(browser) {
    ...
    if (!cookiesLoaded) {
        throw new Error(
            "Not logged in to LinkedIn. Please run: research login",
        );
    }
...
export async function scrapeLinkedInProfile(url, options = {}) {
    ...
    if (!isValid) {
        if (options.autoRelogin) {
            logger.info(
                ...
                "session invalid — attempting auto-relogin",
            );
            const loginSuccess = await relogin();
            if (!loginSuccess) {
                throw new Error(
                    "Auto-relogin failed. Please run: node src/index.js login",
                );
            }
        } else {
            throw new Error(
                "LinkedIn session expired. Please run: node src/index.js login",
            );
        }
    }
    ...
    logger.info(
        { event: "profile.scrape.start", url },
        ...
        const profileData = await page.evaluate(() => {
            ...
            allSections.forEach((section) => {
                ...
                if (
                    ...
                    (sectionText.includes("license") ||
                        sectionText.includes("certification"))
                ) {
            ...
            if (sections.header) {
                ...
                const connectionsRegex =
                    /\d+\+?\s*(connections?|followers?|mutual)/i;
...
export async function searchLinkedIn(query, type = "people", options = {}) {
    ...
    try {
        ...
        logger.debug(
            { event: "search.navigate", searchUrl },
        ...
        try {
            ...
            await page
                .screenshot({ path: "/tmp/linkedin-error.png" })
                .catch(() => {});
            logger.error(
                {
                    event: "search.nav_failed",
                    err: navError.message,
                    currentUrl,
                    screenshot: "/tmp/linkedin-error.png",
                },
                "navigation failed",
            );
            throw navError;
        ...
        const currentUrl = page.url();
        logger.debug(
            { event: "search.landed", pageTitle, currentUrl },
            "arrived at page",
        );
        ...
        await page
            .waitForSelector(".search-results-container", { timeout: 10000 })
            .catch(() => {
                logger.warn(
                    { event: "search.results_container_missing" },
                    "search results container not found, continuing anyway",
                );
            });
        ...
        const pageContent = await page.evaluate(() => {
            ...
            if (!main)
                return { raw_text: document.body.innerText, profile_links: [] };
        ...
        logger.debug(
            {
                event: "search.extracted",
                itemCount: pageContent.structured_items?.length || 0,
            },
