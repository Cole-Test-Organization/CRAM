...
- **Created:** 2026-06-08
- **Status:** **Phases 0–2 complete** (Phase 0 2026-06-08; Phases 1–2 2026-06-15). The storage migration is done: broker ported to `api/src/services/provisioning/`; config + runtime state + jobs all behind Postgres repositories; secrets encrypted at rest; Terraform on the native `pg` backend; the discovery-only config seam closed. **Phase 3 (expose across five surfaces + DB-claim worker) is next.** Work is on branch `provisioning-migration` (off `main`); `tsc --noEmit` green across the api; migrations + secrets + state-repo round-tripped against a throwaway PG (config-repo/seam, resolver, terraform `pg` backend, and seed are written + typecheck clean but not yet runtime-verified — see the Phase 2 checkpoint + smoke plan). Targets: **AWS-first**, **greenfield**.
