...
const REFS: Record<string, OpRef> = {
  ...
  'accounts.clear_review': { http: 'POST /api/accounts/clear-review',           mcp: { tool: 'accounts', action: 'clear_review' } },
  ...
  // org chart — account-scoped reporting graph over account_contacts
  'org_chart.get':         { http: 'GET /api/accounts/:accountId/org-chart',                       mcp: { tool: 'org_chart', action: 'get' } },
  'org_chart.set_manager': { http: 'PATCH /api/accounts/:accountId/org-chart/contacts/:contactId', mcp: { tool: 'org_chart', action: 'set_manager' } },
  'org_chart.remove':      { http: 'DELETE /api/accounts/:accountId/org-chart/contacts/:contactId', mcp: { tool: 'org_chart', action: 'remove' } },
  'org_chart.replace':     { http: 'PUT /api/accounts/:accountId/org-chart',                       mcp: { tool: 'org_chart', action: 'replace' } },
  ...
  // news (per-account Google News headlines, LLM-ranked; manual refresh + daily auto-refresh for favorites)
  'news.get':                   { http: 'GET /api/accounts/:id/news',          mcp: { tool: 'news', action: 'get' } },
  'news.refresh':               { http: 'POST /api/accounts/:id/news/refresh', mcp: { tool: 'news', action: 'refresh' } },
  'news.update_account_prompt': { http: 'PATCH /api/accounts/:id/news',        mcp: { tool: 'news', action: 'update_account_prompt' } },
  'news.get_settings':          { http: 'GET /api/news/settings',              mcp: { tool: 'news', action: 'get_settings' } },
  'news.update_settings':       { http: 'PATCH /api/news/settings',            mcp: { tool: 'news', action: 'update_settings' } },
  ...
  'provisioning.check_credentials':{ http: 'GET /api/provisioning/credentials',                                   mcp: { tool: 'provisioning', action: 'check_credentials' } },
  'provisioning.reconcile':        { http: 'GET /api/provisioning/reconcile (dry run) · POST /api/provisioning/reconcile (apply)', mcp: { tool: 'provisioning', action: 'reconcile' } },
  'provisioning.last_reconcile':   { http: 'GET /api/provisioning/reconcile/latest',                                mcp: { tool: 'provisioning', action: 'last_reconcile' } },
...
This API manages sales account data: accounts (split into "account" — companies you sell to — and "partner" — channel partners you sell with), contacts, account-scoped org charts, meetings, internal notes, opportunities (deals tied to non-partner accounts), a per-user product catalog (products + categories) for what the user sells, a global vendor catalog (vendors + vendor_products) for what accounts run, an account_details record per account capturing the typed technical profile (firmographics + vendor product references + notes), and tasks. It's the source of truth for all CRM-adjacent data. Most rows live behind per-user row-level security (you only see your own); the vendor catalog is shared globally so analytics and dedupe work across users.
...
- **Org charts** are account-scoped placements over the account's external contacts. An associated contact is unassigned until explicitly placed: no chart row means "not in chart", a row with no manager means top-level, and a row with a manager is a reporting relationship. Use ${ref('org_chart.get')} to read the full contact pool separately from explicit chart nodes and edges, ${ref('org_chart.set_manager')} to place someone under a manager or explicitly top-level, ${ref('org_chart.remove')} to remove a leaf from the chart, and ${ref('org_chart.replace')} to replace the full structure. Moving a manager preserves their reports beneath them. Do not store reporting lines on \`contacts\`: the same person can be linked to multiple accounts, so reporting relationships belong to the account/contact membership. Internal support-team contacts are excluded from org charts.
...
- **News** is a per-account feed of recent Google News headlines (headline + link + source + date only — no article bodies), ranked (not rewritten) by the configured local LLM. Read-only until refreshed; see **News** below.
...
- **Contacts — prefer ${ref('contacts.find_or_create')}** over plain create. It's the single creation path, running full dedupe + enrich: a supplied email is trimmed/lowercased and is unique per user when nonblank; matching then proceeds by normalized email → exact \`full_name\`+\`kind\` → fuzzy \`full_name\` within the same kind (pg_trgm). It returns the match (\`matched_by\`, plus \`match_score\` when fuzzy), (re)links it to \`account_id\` when given, and — when you supply a value for a field that's currently BLANK on the stored contact — fills it in (reported via \`enriched\`/\`enriched_fields\`), never overwriting existing data. Send at least an email OR a name: **name-only and email-only contacts are both valid**, and multiple name-only contacts may have NULL email. Concurrent imports of the same email converge on one row. This keeps you from piling up near-duplicate contacts, especially \`kind=internal\` teammates. ${ref('contacts.find_existing')} is the read-only probe (returns null, no write); ${ref('contacts.create_standalone')} runs the same dedupe but still refuses with 409 + an \`existing\` payload on a match.
...
- **Unknown company** → a new account is **auto-created with \`accounts.needs_review=true\`** and the note linked to it; the meeting itself is not flagged unless its placement is uncertain. Review the account queue via ${ref('accounts.list_full')} with \`needs_review=true\`; clear the account flag with ${ref('accounts.update')} once verified, or ${ref('accounts.clear_review')} to approve a batch at once (pass \`ids\` for a specific set, omit it to approve the whole queue). The flag means "this row was minted automatically, nobody has confirmed it" — not "this row is wrong"; the meeting that triggered the auto-create is normally on the right account already.
- **Ambiguous** (a similar account already exists) → the note is **parked** (\`internal=true, needs_review=true, review_reason:"account_ambiguous"\`, no account) rather than risk a duplicate account. Find parked notes via ${ref('meetings.list')} with \`needs_review=true\` and place them with ${ref('meetings.assign_account')}.
...
### News
Per-account news headlines pulled from Google News RSS for the account's **company name** (not the slug) and ranked — not rewritten — by the user's configured local LLM. Only headline + link + source + date are stored; there is no article body to read.
- **Read is pure.** ${ref('news.get')} returns the stored ranked headlines plus \`status\` (null = never fetched, \`refreshing\`, \`ok\`, \`error\`) and \`last_fetched_at\`. It never fetches — opening an account doesn't silently spend a local-LLM call.
- **Refresh is manual + async.** ${ref('news.refresh')} fetches fresh headlines and re-ranks them; it returns \`status:"refreshing"\` immediately (the local LLM can take 10–30s), then poll ${ref('news.get')} until \`status\` is \`ok\`/\`error\`. Ranking failures fall back to raw feed order rather than dropping the news.
- **Starred accounts auto-refresh.** Accounts the user has favorited ("starred") are refreshed once each morning by the scheduler; others refresh only when asked. Don't loop ${ref('news.refresh')} across every account — it hits an external feed and the local model.
- **Ranking prompt.** Ordering is driven by a prompt with a built-in default. ${ref('news.get_settings')} / ${ref('news.update_settings')} manage the **global** prompt (null/empty reverts to the built-in default); ${ref('news.update_account_prompt')} sets a **per-account override** (null falls back to the global). Only touch these when the user asks how their news is prioritized.
...
An in-process broker that stands up homelab/cloud infrastructure (Terraform + PAN-OS lifecycle across AWS, GCP, and Proxmox), ported from panw-broker. This is the single user's lab — not per-tenant CRM data. Terraform state lives in Postgres (native \`pg\` backend), runtime state + jobs in the \`provisioning_*\` tables.
...
- **Secrets resolve by name.** Deployment config references a secret by env-var name (e.g. \`PANW_PANORAMA_AUTH_CODE\`); store the value once with ${ref('provisioning.set_secret')} (AES-256-GCM at rest). Most values are write-only, and ${ref('provisioning.list_secrets')} returns only a server-computed \`valueSuffix\` for them. The non-secret allowlist returns dashboard-readable \`value\` fields: \`PANW_DEVICE_CERT_PIN_ID\`, \`PANW_NGFW_AUTH_CODE\`, \`PANW_PANORAMA_AUTH_CODE\`, \`PANW_PANORAMA_SERIAL\`. Satisfy a deployment's \`requiredEnv\` before deploying.
...
- **Run multiple copies of one blueprint with instances.** A seeded deployment is a *template* (\`list_deployments\` shows it with \`isTemplate:true\`). To stand up several isolated copies of the same blueprint **without editing config files**, first inspect its declared \`inputs\` with ${ref('provisioning.get_deployment')}, then call ${ref('provisioning.create_instance')} with an operator \`name\` and any chosen input values in \`params\`. The broker applies declared launch inputs to the cloned resource configuration before enqueueing its deploy. For \`aws-single-firewall\`, use \`managementMode:"scm"\` plus a non-empty \`scmFolder\` to enroll in Strata Cloud Manager; it requires the \`PANW_DEVICE_CERT_PIN_ID\` and \`PANW_DEVICE_CERT_PIN_VALUE\` secrets and waits for cloud-management connectivity. The default \`managementMode:"direct"\` keeps the standalone baseline. Each clone gets unique Terraform workspaces and cloud resource names, so nothing collides; its job returns the new \`deployment\` slug. Instances carry a \`templateName\` and survive the boot reseed. Manage an instance with the normal \`deploy\`/\`deprovision\`/\`down\` verbs against its slug; once its resources are destroyed, ${ref('provisioning.delete_instance')} removes the row.
...
- **Recorded state can be stale — reconcile before you trust it.** Broker state records what the broker provisioned, so anything torn down *outside* the broker (a console click, a CLI sweep, an expired lab) still reads as live. ${ref('provisioning.reconcile')} asks each provider whether every tracked resource still exists and returns a per-resource \`status\`: \`present\`, \`missing\` (the provider confirmed it is gone — \`stale:true\` when the record still claimed it was live), \`credentials-invalid\`, \`unknown\`, or \`unsupported\`. **\`credentials-invalid\` is not evidence about the infrastructure** — the login expired, so nothing could be checked. The GET/no-\`apply\` form is a dry run; \`apply:true\` writes confirmed-gone resources back as \`destroyed\` (which also unblocks ${ref('provisioning.delete_instance')}), and never touches resources whose credentials failed. **Reconciling is expensive** — one cloud CLI call per tracked resource, run serially — so it is not something to poll. Every run is stored, a background sweep refreshes it on an interval (\`RECONCILE_INTERVAL_MINUTES\`, default 15, read-only — it never applies), and ${ref('provisioning.last_reconcile')} returns that stored answer with no cloud calls. **Read ${ref('provisioning.last_reconcile')} first** and judge staleness from its \`checkedAt\`; only run a fresh ${ref('provisioning.reconcile')} when you need an answer newer than that, or when you are about to act on the result. A deployment-scoped run is stored separately from a whole-broker one and never answers for it.
- **Credentials expire often — check them first.** ${ref('provisioning.check_credentials')} probes every configured provider profile once per credential scope (account + region + profile) and reports \`state\`: \`ok\`, \`expired\` (re-login, e.g. \`aws sso login\` / \`gcloud auth login\`), \`missing\` (never configured), \`denied\` (authenticated but unauthorized), \`error\`, or \`unsupported\`, each with the identity it resolved to and a \`remediation\` hint. When a deploy or a reconcile behaves oddly, check this before concluding anything about the lab.
- **Runtime tunnels are broker-managed sessions.** For private AWS Windows endpoints, use ${ref('provisioning.open_rdp_tunnel')} to start an SSM-backed RDP tunnel on a Docker-published LAN port. For private Linux endpoints, use ${ref('provisioning.open_ssh_tunnel')} to start a tunnel (SSM on AWS, IAP TCP forwarding on GCP); the returned view includes an \`sshCommand\`. Use ${ref('provisioning.list_tunnels')} to see active endpoints and ${ref('provisioning.close_tunnel')} to close one. These are process-local sessions, not Terraform resources; reopen after an API/container restart.
- **Providers differ in what they support.** AWS is the most complete (PAN-OS VM-Series/Panorama, Windows + Ubuntu endpoints, SSM shell/RDP/SSH). GCP covers \`network\` and \`ubuntu-server\` — deployments \`gcp-shared-network\` and \`gcp-ubuntu-server\`, IAP instead of SSM, \`gcloud\` for power/credentials/existence; there is no GCP Windows endpoint or VM-Series stack, and \`verify-internet-access\` (an SSM marker probe) is skipped on GCP. Proxmox provisions through its own API rather than Terraform. A resource kind is only deployable on a provider that has a \`<provider>-<kind>\` terraform profile.
...
Partners are managed via the dedicated partner endpoints (\`list_partners\`, \`add_partner\`, \`remove_partner\`) — not through the account body. The supporting team is managed the same way contacts always are: link \`kind=internal\` contacts to the account (${ref('contacts.link')} / ${ref('contacts.unlink')}); they come back in the account's \`team\` array, not as an account field. Org-chart reporting relationships are managed separately via ${ref('org_chart.set_manager')} / ${ref('org_chart.replace')}. **Technical environment data does not live on the account record** — it's on \`account_details\`, see that section's update semantics (scalars replaced when present; array fields fully replaced when present).`;
