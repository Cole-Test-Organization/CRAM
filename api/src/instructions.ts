...
- **Org charts** are account-scoped reporting edges over the account's external contacts. Use ${ref('org_chart.get')} to read all nodes and edges, ${ref('org_chart.set_manager')} to set/clear one person's manager, and ${ref('org_chart.replace')} to replace the full edge set. Do not store reporting lines on \`contacts\`: the same person can be linked to multiple accounts, so reporting relationships belong to the account/contact membership. Internal support-team contacts are excluded from org charts.
