...
- **Discover, then act.** ${ref('provisioning.list_deployments')} lists what's deployable; ${ref('provisioning.get_deployment')} returns a deployment's resources, ordered steps, launch inputs, and **\`requiredEnv\`** — the secret names it needs.
...
- **RDP tunnels are broker-managed runtime sessions.** For private AWS Windows endpoints, use ${ref('provisioning.open_rdp_tunnel')} to start an SSM-backed RDP tunnel on a Docker-published LAN port, ${ref('provisioning.list_tunnels')} to see active endpoints, and ${ref('provisioning.close_tunnel')} to close one. These are process-local sessions, not Terraform resources; reopen after an API/container restart.
