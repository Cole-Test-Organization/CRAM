...
const MEETING_REVIEW_REASON_VALUES = [
  'manual',
  'account_unassigned',
  'account_ambiguous',
...
export function registerTools(server: McpServer, services: Services, resolveUserId: ResolveUserId) {
  ...
  server.tool(
    'provisioning',
    'Provision and manage homelab/cloud infrastructure (the ported panw-broker: Terraform + PAN-OS lifecycle across AWS, GCP, and Proxmox). Discovery and reads are immediate; lifecycle verbs are **async** — they enqueue a durable job a background worker runs, so you get a job back and must poll `get_job` until status is `succeeded`/`failed`/`canceled` (logs stream into the job). Actions: list_deployments (available deployment summaries), get_deployment (resources, steps, inputs, and `requiredEnv` secret names), list_resources / get_resource (runtime state), discover_proxmox (Proxmox cluster inventory — nodes/templates/datastores/bridges, to fill in a Proxmox deployment), check_credentials (are each cloud\'s credentials still usable — ok/expired/missing/denied), reconcile (drift check against the real cloud: which tracked resources still exist, which are gone; `apply:true` writes the gone ones back as destroyed — SLOW, one cloud CLI call per resource, serially), last_reconcile (the stored report from the last run at this scope, no cloud calls — prefer this and read its `checkedAt` for staleness; a background sweep refreshes it every RECONCILE_INTERVAL_MINUTES, default 15, and only `reconcile` forces a fresh one), event_snapshot (active job + resources + recent jobs baseline; GUI clients subscribe to HTTP SSE for live updates), power_state (refresh from the cloud), start / stop (quick power toggle — refused while a job runs), list_tunnels / open_rdp_tunnel / open_ssh_tunnel / close_tunnel (broker-managed LAN TCP tunnel sessions over SSM: RDP for Windows endpoints, SSH for Linux endpoints), deploy / deprovision (deploy or tear down a whole deployment; deployments without steps deploy their configured resources), create_instance (clone a template deployment under a new operator-chosen name, apply its declared launch inputs to that clone, and deploy it — the way to run multiple isolated copies of one blueprint without editing config files) / delete_instance (remove a torn-down instance row), up (one resource) / down (destroy one resource), run_action (a resource-specific action like verify-connected-resources), list_jobs / get_job / cancel_job, list_secrets / set_secret / delete_secret (encrypted at rest; most values are write-only and return only a server-computed `valueSuffix`, while allowlisted non-secret values return a `value` for dashboard display), seed (idempotently seed the typed code modules in config/modules/**). A deployment must be seeded before you can deploy/up it; in `list_deployments`, rows with `isTemplate:true` are launchable blueprints and rows with a `templateName` are deployed instances. Secrets needed by a deployment are its `requiredEnv`. **If a resource looks live but you suspect it was already torn down, run `reconcile` before believing the recorded state** — and read its `credentials` block first: a `credentials-invalid` resource means the cloud login expired, NOT that the machine is gone.',
    {
      action: z.enum(['list_deployments', 'get_deployment', 'list_resources', 'get_resource', 'discover_proxmox', 'check_credentials', 'reconcile', 'last_reconcile', 'event_snapshot', 'power_state', 'start', 'stop', 'list_tunnels', 'open_rdp_tunnel', 'open_ssh_tunnel', 'close_tunnel', 'deploy', 'deprovision', 'create_instance', 'delete_instance', 'up', 'down', 'run_action', 'list_jobs', 'get_job', 'cancel_job', 'list_secrets', 'set_secret', 'delete_secret', 'seed']),
      deployment: z.string().optional().describe('Deployment slug — the template for create_instance, the instance for delete_instance, or the target for get_deployment/deploy/deprovision/up/run_action. Optional filter for reconcile.'),
      ...
      target: z.string().optional().describe('Resource hostname/name/id — for get_resource, power_state, start, stop, up, down, run_action, open_rdp_tunnel, open_ssh_tunnel, close_tunnel by resource.'),
      ...
      value: z.string().optional().describe('Secret value — for set_secret (encrypted at rest; returned only for allowlisted non-secret names).'),
      ...
      port: z.number().int().positive().optional().describe('LAN-facing tunnel port from PROVISIONING_TUNNEL_PORTS / PROVISIONING_RDP_TUNNEL_PORTS — for open_rdp_tunnel/open_ssh_tunnel.'),
      remote_port: z.number().int().positive().optional().describe('Remote resource port; defaults to 3389 for open_rdp_tunnel and 22 for open_ssh_tunnel.'),
      ttl_seconds: z.number().int().nonnegative().optional().describe('Seconds before the broker closes the tunnel; 0 disables TTL.'),
      apply: z.boolean().optional().describe('reconcile only: write vanished resources back as `destroyed`. Default false (dry-run report).'),
      include_destroyed: z.boolean().optional().describe('reconcile only: also probe records already marked destroyed. Default false.'),
      params: z.record(z.any()).optional().describe('Declared launch inputs and step toggles (`when` params). For create_instance, declared inputs are persisted into the cloned deployment before it is deployed; for deploy/deprovision/up/down/run_action they control that job only.'),
    },
    async ({ action, deployment, instance_name, target, resource_action, tunnel_id, job_id, name, value, description, status, limit, port, remote_port, ttl_seconds, apply, include_destroyed, params }) => {
      switch (action) {
        ...
        case 'discover_proxmox':
          return callService(() => provisioningService.discoverProxmox());
        case 'check_credentials':
          return callService(() => provisioningService.checkCredentials());
        case 'reconcile':
          return callService(() => provisioningService.reconcile({
            apply: apply === true,
            deployment: deployment ?? null,
            includeDestroyed: include_destroyed === true,
          }));
        case 'last_reconcile':
          return callService(() => provisioningService.lastReconcile(deployment ?? null));
        case 'power_state':
        ...
        case 'list_tunnels':
          return callService(() => provisioningService.listTunnels());
        ...
        case 'open_ssh_tunnel':
          if (!target) return errorResponse('open_ssh_tunnel requires `target` (Linux resource id/hostname/name).');
          return callService(() => provisioningService.openSshTunnel(target, {
            port,
            remotePort: remote_port,
            ttlSeconds: ttl_seconds,
          }));
        case 'close_tunnel': {
          ...
          return callService(() => provisioningService.closeTunnel(id), { notFoundMsg: `No tunnel "${id}".` });
        }
        case 'deploy':
  ...
  server.tool(
    ...
    async ({ action, id, ids, partner_id, slug, domain, status, exclude_status, needs_review, create_if_missing, fuzzy, sort, order, limit, offset, data }) => {
      ...
      switch (action) {
        ...
        case 'clear_review':
          return callService(() => accountsService.clearReviewFlags(userId, Array.isArray(ids) && ids.length ? ids : null));
        case 'delete':
  ...
  server.tool(
    ...
    async ({ action, id, email, account_id, from_account_id, company, kind, city, country, mode, search, enrichment_job_id, emails, from_emails_payload, limit, offset, data }) => {
      ...
      switch (action) {
        ...
        case 'find_or_create':
          if (!data || (!data.email && !data.full_name)) return errorResponse('find_or_create requires data with at least one of email or full_name (name-only and email-only contacts are valid). A supplied email is trimmed, lowercased, and unique per user. Optionally pass data.kind ("account" default | "partner" | "internal") and account_id to (re)link. Idempotent even under concurrent submissions: returns the existing contact (matched_by "email" | "full_name" | "fuzzy", with match_score for fuzzy) instead of creating when matched, filling any BLANK stored field from what you pass (enriched/enriched_fields) without overwriting existing values — never throws 409. This is the preferred way to add a contact.');
  ...
  server.tool(
    ...
    'Manage meeting notes. Meetings can be tied to an account (account or partner) or internal-only (no account). Attendees come in two forms: LINKED (contact_ids → existing CRM contacts) and UNLINKED (a name/email with no contact yet — recorded for visibility, linkable later). Actions: list (all, paginated; filter by account_id, internal flag, or needs_review), get (by id — body, linked contacts, and unlinked_attendees[]), create (pass account_id + contact_ids for an account meeting; pass internal=true and omit account_id for an internal-only note; needs_review=true parks an uncertain/imported note for triage; review_reason explains why), update (partial — contact_ids replaces the LINKED set, attendees/unlinked_attendees replaces the UNLINKED set, independently; the internal flag and account_id cannot be changed after creation), delete, assign_account (triage — attach a parked account-less note to an account, flipping internal→false and clearing needs_review/review_reason; 409 if already assigned), reassign_account (move a meeting that ALREADY has an account to a different account, or convert it to an internal note — the fix-a-bad-import path; unlike assign_account it is NOT blocked when the meeting already has an account, and clears needs_review/review_reason), link_attendee (triage — convert an unlinked attendee row into a link to an existing contact, deduping if already linked), create_from_emails (atomic: creates the account + contacts AND a meeting from a resolved email list — use ONLY when you have meeting notes/body; to add the account + people WITHOUT a meeting, use the contacts tool actions resolve_emails + import_from_emails, which this delegates the account/people half to. Fires optional background outreach + LLM enrichment for new contacts flagged research:true), get_enrichment_job (poll a single enrichment job kicked off by create_from_emails), list_enrichment_jobs (list all enrichment jobs for the contacts on a given meeting — useful for a progress dashboard).',
    {
      ...
      needs_review: z.boolean().optional().describe('For list: filter by the triage flag (true=only meetings/notes needing review, false=only settled, omit=both). Set on create/update via data.needs_review.'),
      limit: z.number().optional(),
      offset: z.number().optional(),
      ...
      data: z.object({
        ...
        title: z.string().optional(),
        needs_review: z.boolean().optional().describe('Park this meeting/note for triage (create/update). Surfaced by list with needs_review=true; clearing it also clears review_reason.'),
        review_reason: z.enum(MEETING_REVIEW_REASON_VALUES).nullable().optional().describe('Typed reason when needs_review=true: manual, account_unassigned, account_ambiguous, account_auto_created, krisp_no_match, krisp_multiple_matches, or krisp_match_legacy. Setting a non-null reason also sets needs_review=true; null clears review.'),
  ...
  server.tool(
    'notes_import',
    'Bulk-import a directory of notes (markdown/plain text from Obsidian, Apple/Google Notes, a folder of call summaries, …). Each file is processed ONE AT A TIME by the local model to extract metadata (date, title, account, attendees), then resolved to an account and written as a meeting. Account resolution: a confident match links the note cleanly; an unknown company auto-creates a flagged account (accounts.needs_review) and links it; an ambiguous near-match parks the note (internal + needs_review + review_reason=account_ambiguous) so you place it via triage instead of minting a near-duplicate; an internal/no-company note parks with review_reason=account_unassigned. Async + serial (one local-LLM call at a time). Actions: enqueue (pass files=[{path, content}] — read/convert the directory client-side; returns a jobId), get_job (poll by jobId — status queued→running→completed, stage shows progress, results[] has a per-file outcome, counts aggregates them), list_jobs (recent jobs). Idempotent: re-importing the same file is skipped on a filename match. (Uploading a raw .zip is HTTP-only — POST /api/notes-import/upload-zip — since MCP can\'t carry binary; the HTTP zip endpoint extracts text files and converts .docx/text-based .pdf entries, including zipped Google Drive folder downloads, into the same files[] shape before enqueueing.)',
    {
