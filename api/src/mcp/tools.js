...
export function registerTools(server, services, resolveUserId) {
  ...
  server.tool(
    ...
    async ({ action, id, email, account_id, company, kind, city, country, mode, search, enrichment_job_id, limit, offset, data }) => {
      ...
      switch (action) {
        ...
        case 'create':
          if (!data?.full_name) return errorResponse('create requires data.full_name. Also recommended: data.email (used for dedupe later via get_by_email), data.kind ("account" default | "partner" | "internal"), and account_id if linking to an account (kind="account" or "partner" only — internal contacts have no account link). Throws 409 with the existing row attached if a duplicate is detected (email match or full_name+kind match) — call action="find_existing" first if you want to handle that case without an exception.');
  ...
  server.tool(
    'import_export',
    'Portable JSON bundles for moving accounts (with details, contacts, meetings, opportunities, and partner shells) between tenants. Actions: export (returns a bundle JSON for one or more account slugs), import (merges a bundle into the current tenant — accounts matched by slug, contacts by email/name, meetings by filename, opportunities by name; vendor products dedupe globally by slug). Different from `export_markdown`, which produces human-readable docs; this is a round-trippable data export.',
    {
  ...
  server.tool(
    'backup',
    'Database backups (pg_dump custom format). Backups are instance-wide, not per-user — a single dump captures every tenant\'s data. Files land in the bind-mounted target directory (default /backups; the host mount is configured via BACKUP_HOST_DIR in the operator\'s .env). Actions: get_settings, update_settings (PATCH the enabled / cron / retention_count / target_dir fields; saving reschedules the cron job in-process), list (newest first, with filename / size / created_at), run (trigger a dump now), restore (DESTRUCTIVE — pg_restore --clean --if-exists drops and recreates every object; only call when intentionally rolling back), delete (remove a dump file).',
    {
      action: z.enum(['get_settings', 'update_settings', 'list', 'run', 'restore', 'delete']),
    ...
    async ({ action, filename, settings }) => {
      switch (action) {
        case 'get_settings':
