...
import type { CalendarImportService } from '../../services/calendar-import/calendar-import.js';

const BODY_LIMIT = Number(process.env.CALENDAR_IMPORT_BODY_LIMIT) || 16 * 1024 * 1024; // 16MB

export default async function calendarImportRoutes(fastify: FastifyInstance, { calendarImportService }: { calendarImportService: CalendarImportService }) {
  const expectedToken = process.env.CALENDAR_IMPORT_TOKEN || null;

  fastify.post<{ Body: Record<string, unknown> }>('/calendar-import', {
    bodyLimit: BODY_LIMIT,
    schema: {
      description: 'Import a day of Google Calendar events (the JSON your daily export forwards through the tunnel). For each event whose RSVP is not "Declined" (all-day events, and organizer-only holds with no other guests, skipped by default): attendee emails are classified by domain — internal domains → kind=internal contacts (no account link); a domain matching a status=partner account (or listed in CALENDAR_PARTNER_DOMAINS) → kind=partner; personal/freemail domains (gmail, yahoo, …) are skipped; any other business domain → kind=account. The meeting attaches to the most-attended CUSTOMER domain (internal/partner never decide it); with no customer domain it becomes an internal note. Unknown customer domains auto-create an account flagged accounts.needs_review; the meeting itself is not flagged unless its own placement is uncertain. The calendar description (HTML) is converted to markdown and prepended to the meeting body. Synchronous — returns a per-event report. Idempotent: re-sending a day is a no-op (meetings are keyed on the calendar event id). If CALENDAR_IMPORT_TOKEN is configured, send it as the x-calendar-import-token header.',
          ...
          meetings: {
            ...
            items: {
              ...
              properties: {
                ...
                guestCount: { type: 'integer', description: 'Number of guests. An event with no guest other than the organizer (you) is treated as a calendar block and skipped unless CALENDAR_IMPORT_SOLO=true.' },
