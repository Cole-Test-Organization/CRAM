...
export default async function backupRoutes(fastify: FastifyInstance, { backupService }: { backupService: BackupService }) {
  ...
  fastify.get('/backup/settings', {
    schema: {
      description: 'Get the current backup configuration (retention_count, target_dir).',
      tags: ['backup'],
    },
  }, async () => {
  ...
  fastify.put<{ Body: { retention_count?: number; target_dir?: string } }>('/backup/settings', {
    schema: {
      description: 'Update the backup configuration. PATCH-merge — only fields you send are changed. Validates the retention count and target_dir (must be an absolute path inside the container — the host mount lives at /backups by default, configurable via the BACKUP_HOST_DIR env var on the host).',
      tags: ['backup'],
      body: {
        type: 'object',
        properties: {
  ...
  fastify.post<{ Body: any; Querystring: { filename?: string } }>('/backup/import', {
    schema: {
  ...
  fastify.post<{ Body: { path: string } }>('/backup/import-from-path', {
    schema: {
  ...
  fastify.post<{ Body: { filename: string } }>('/backup/restore', {
    schema: {
  ...
  fastify.get<{ Params: { filename: string } }>('/backup/download/:filename', {
    schema: {
