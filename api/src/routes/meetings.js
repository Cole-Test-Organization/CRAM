    ...
    schema: {
      ...
      body: {
        ...
        properties: {
          ...
          starts_at: { type: 'string', format: 'date-time', description: 'Optional precise start as an ISO 8601 timestamp (e.g. "2026-05-31T13:30:00Z"). Powers the GUI Today timeline and time-of-day ordering; the calendar import sets it from the event start. Distinct from `date` (the calendar day).' },
          ends_at: { type: 'string', format: 'date-time', description: 'Optional precise end as an ISO 8601 timestamp. Companion to starts_at; used to detect the meeting happening right now.' },
          location: { type: 'string', description: 'Optional location — for a virtual meeting the conferencing URL (Google Meet / Zoom / Teams), which the Today timeline renders as a one-click "Join" button; for in-person, a room or address. The calendar import sets it from the event location.' },
    ...
    schema: {
      ...
      body: {
        ...
        properties: {
          ...
          starts_at: { type: 'string', format: 'date-time', description: 'Precise start as an ISO 8601 timestamp. Omit to leave unchanged; null clears it.' },
          ends_at: { type: 'string', format: 'date-time', description: 'Precise end as an ISO 8601 timestamp. Omit to leave unchanged; null clears it.' },
          location: { type: 'string', description: 'Location / conferencing URL. Omit to leave unchanged; null clears it.' },
          title: { type: 'string' },
