...
- **Contacts** carry a \`kind\`: \`account\` (works at a non-partner account — link them), \`partner\` (channel/reseller rep — link to a partner account), or \`internal\` (teammate at your own company — no account link). Default is \`account\`. Account links are many-to-many: ${ref('contacts.link')} / ${ref('contacts.unlink')} manage the set, and ${ref('contacts.reassign')} atomically moves a contact from one account to another (links the destination, then unlinks the source — fixing a contact imported onto the wrong account without a window where it's orphaned). It only moves the one link you name; any other account links are preserved.
