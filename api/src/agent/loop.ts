...
function buildUserContent(prompt: string, notes: string | undefined, taggedBlock: string) {
    let out = prompt;
...
function stringifyMcpResult(result: any) {
...
export async function runAgent({
    ...
    try {
        ...
        logger.warn(
            ...
            `mention resolution failed: ${(err as Error)?.message || err}`,
        );
    }
    ...
    const mcpTools = Array.isArray(allowedTools)
        ? allMcpTools.filter((t: any) => allowedTools.includes(t.name))
        : allMcpTools;
    ...
    try {
        while (iter < MAX_ITERATIONS) {
            ...
            if (toolCalls.length === 0) {
                ...
                const hasVisibleText = content.some(
                    (b: any) => b.type === "text" && b.text && b.text.trim(),
                );
            ...
            for (const call of toolCalls) {
                ...
                try {
                    ...
                    text = `Tool error: ${(err as Error).message || String(err)}`;
                    isError = true;
                }
                send({
                    type: "tool_result",
        ...
        try {
            ...
            send({
                type: "error",
                message: `Failed to save session: ${(err as Error).message || String(err)}`,
            });
        }
    }
