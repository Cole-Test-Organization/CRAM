...
const isClosed = (stage: any) => typeof stage === "string" && stage.endsWith("_closed");
const day = (d: any) => (d ? String(d).slice(0, 10) : null);

function fmtAccount(a: any) {
    const parts = [a.name];
    ...
    const open =
        typeof a.active_deals === "number"
            ...
            : Array.isArray(a.opportunities)
              ? a.opportunities.filter((o: any) => !isClosed(o.stage)).length
              : null;
    ...
    const last =
        ...
        (Array.isArray(a.meetings)
            ? day(a.meetings.map((m: any) => m.date).filter(Boolean).sort().at(-1))
            : null);
...
async function resolveOne(userId: number, m: Mention, services: Services): Promise<string | null> {
 ...
 */
export async function resolveMentions(userId: number, mentions: Mention[] | null | undefined, services: Services): Promise<string> {
    ...
    for (const m of capped) {
        try {
            ...
            lines.push(`[${m?.type} #${m?.id}] ${m?.label ?? ""} — (lookup failed: ${(err as Error)?.message || "error"})`);
        }
    }
