...
// downstream services that read `content` (contact-enrichment, notes-import)
// stay assignable to their own narrower casts.
  ...
  try {
    ...
    if (res.ok) {
      ...
      models = Array.isArray(json?.data)
        ? json.data.map((m: any) => m?.id).filter((id: any) => typeof id === 'string' && id)
        : [];
    }
  } catch {
    models = [];
...
async function probeOllama(baseUrl: string, headers: Record<string, string>, model?: string): Promise<number | null> {
...
export async function streamTurn(args: StreamTurnArgs): Promise<StreamTurnResult | undefined> {
  for (let attempt = 1; attempt <= MAX_STREAM_ATTEMPTS; attempt++) {
    try {
      return await streamTurnOnce(args);
    } catch (err) {
      const code = (err as { cause?: { code?: string }; message?: string })?.cause?.code || (err as Error)?.message;
...
async function streamTurnOnce({ model, system, messages, mcpTools, onBlock, providerConfig, timeoutMs, think }: StreamTurnArgs): Promise<StreamTurnResult> {
  ...
  function consumeContent(chunk: string) {
    tagPending += chunk;
    while (true) {
      if (inThink) {
  ...
  if (thinkingBuf) {
    const block: ContentBlock = { type: 'thinking', thinking: thinkingBuf };
    content.push(block);
    onBlock?.(block);
  }

  if (textBuf) {
    const block: ContentBlock = { type: 'text', text: textBuf };
    content.push(block);
    onBlock?.(block);
  }
  ...
  return { stopReason: FINISH_REASON_MAP[finishReason as any] || finishReason, content, usage: usageOut };
}
