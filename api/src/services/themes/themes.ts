...
function validateThemeData(theme_data: any) {
...
export class ThemesService {
  ...
  async create(userId: number, { slug, name, description, theme_data }: { slug?: string; name?: string; description?: string | null; theme_data?: any }) {
    validateSlug(slug);
  ...
  async update(userId: number, id: number, patch: { name?: string; description?: string | null; slug?: string; theme_data?: any }) {
    return withUser(userId, async (client) => {
      ...
      const fields: { name?: string; description?: string | null; slug?: string; theme_data?: any } = {};
  ...
  async setActive(userId: number, themeId: number | string | null | undefined) {
    if (themeId !== null && themeId !== undefined) {
      const n = parseInt(String(themeId), 10);
...
async function readActive(client: PoolClient) {
