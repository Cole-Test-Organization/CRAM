...
function deriveTitle(text: string | null | undefined) {
  if (!text) return null;
...
export function messagesToEvents(messages: any[]) {
  const events = [];
...
export async function createSession(userId: number, { provider = DEFAULT_PROVIDER, model = FALLBACK_MODEL }: { provider?: string; model?: string } = {}) {
  const id = randomUUID();
...
export async function loadSessionRaw(userId: number, sessionId: string) {
  return withUser(userId, async (client) => {
...
export async function loadSession(userId: number, sessionId: string) {
...
export async function saveMessages(userId: number, sessionId: string, messages: any[], derivedTitle: string | null = null) {
...
function buildSnippet(haystack: string, query: string) {
...
export async function listSessions(userId: number, { limit, search }: { limit?: number; search?: string } = {}) {
