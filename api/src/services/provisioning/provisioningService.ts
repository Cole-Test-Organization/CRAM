...
export class ProvisioningService {
    ...
    // ── Proxmox discovery (inline read against the LAN Proxmox API) ──────────────
    // Lists nodes/templates/datastores/bridges so a Proxmox deployment can be filled
    // in. Resolves PROXMOX_VE_ENDPOINT/PROXMOX_VE_API_TOKEN from the encrypted secrets
    // (then process.env) directly — discovery isn't a job, so the global secret overlay
    // isn't installed; touching it here could clobber a concurrent job's overlay.
    async discoverProxmox(): Promise<ProxmoxDiscovery> {
        const secrets = await this.secretResolver.hydrateAll();
        const pick = (key: string): string | undefined =>
            secrets[key] ?? process.env[key];
        const endpoint = pick("PROXMOX_VE_ENDPOINT");
        const apiToken = pick("PROXMOX_VE_API_TOKEN");
        if (!endpoint || !apiToken) {
            throw httpError(
                400,
                "Proxmox discovery needs PROXMOX_VE_ENDPOINT and PROXMOX_VE_API_TOKEN — set them on the Secrets page (or .env) first",
            );
        }
        const insecureRaw = pick("PROXMOX_VE_INSECURE");
        const insecure =
    ...
    async refreshPowerState(target: string): Promise<ResourceRecord> {
        return this.broker.refreshResourcePowerState(target, NOOP_LOG, {
    ...
    async seedSecretsFromEnv(
        ...
        return seedProvisioningSecrets(this.userId, {
            ...options,
    ...
    async enqueueJob(input: EnqueueJobInput): Promise<JobView> {
        ...
        if (!JOB_KINDS.has(kind)) {
            throw httpError(
                400,
                `unknown job kind "${kind}" — use ${[...JOB_KINDS].join(", ")}`,
            );
        }
        ...
        if (
            (kind === "up" || kind === "down" || kind === "run-action") &&
            !target
        ) {
        ...
        if (deployment) {
            ...
            if (deploymentId == null) {
                throw httpError(
                    400,
                    `deployment "${deployment}" is not seeded — POST /api/provisioning/seed first`,
                );
            }
        }
        if (kind === "down") {
    ...
    async listJobs(
        ...
        return withUser(this.userId, async (c) => {
            const res = options.status
                ? await c.query(
                      `${SELECT_JOB} WHERE j.status = $1 ORDER BY j.created_at DESC LIMIT $2`,
    ...
    async requestCancel(id: string): Promise<JobView | null> {
        const found = await withUser(this.userId, async (c) => {
           ...
           WHERE id = $1`,
                ...
                await c.query(
                    `UPDATE broker_state SET active_job_id = NULL WHERE active_job_id = $1`,
                    [id],
                );
            } else if (status === "running") {
                await c.query(
                    `UPDATE provisioning_jobs SET cancel_requested = TRUE WHERE id = $1`,
                    [id],
                );
            }
            return true;
    ...
    private async assertIdle(): Promise<void> {
        ...
        if (state.activeJobId) {
            throw httpError(
                409,
                `a provisioning job is active (${state.activeJobId}); retry once it finishes`,
            );
        }
    }
}
