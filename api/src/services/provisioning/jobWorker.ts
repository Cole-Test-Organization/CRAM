...
export class ProvisioningJobWorker {
    ...
    async start(): Promise<void> {
        ...
        if (recovered > 0)
            logger.warn(
                { recovered },
                "failed orphaned running jobs from a previous process",
            );
        this.running = true;
        this.loopPromise = this.loop();
        logger.info(
            { workerId: this.workerId },
            "provisioning job worker started",
        );
    }
    ...
    private async runJob(row: Record<string, unknown>): Promise<void> {
        ...
        try {
                ...
                void this.isCancelRequested(job.id)
                    .then((requested) => {
                        if (requested && !canceled) {
                            canceled = true;
                            log(
                                "Cancellation requested — terminating the running step…",
                            );
                            abort.abort();
                        }
                    })
            ...
            progressTimer = setInterval(publishProgress, this.progressEventMs);

            logger.info(
                { jobId: job.id, action: job.action, target: job.hostname },
                "running provisioning job",
            );
    ...
    private async dispatch(
        ...
        const options = {
            params: spec.runParams ?? undefined,
            skipActiveJobCheck: true,
        };
        const deployment = spec.deploymentRef ?? undefined;
        switch (job.action) {
            case "deploy":
                await this.broker.deploy(
                    requireRef(deployment, "deploy"),
                    log,
                    options,
                );
                return;
            case "deprovision":
                await this.broker.deprovision(
                    requireRef(deployment, "deprovision"),
                    log,
                    options,
                );
                return;
            case "up":
                await this.broker.up(
                    requireRef(deployment, "up"),
                    log,
                    requireTarget(job.hostname, "up"),
                    options,
                );
                return;
            case "down":
                await this.broker.down(
                    requireTarget(job.hostname, "down"),
                    log,
                    options,
                );
                return;
    ...
    private async recoverOrphans(): Promise<number> {
        return withUser(this.userId, async (c) => {
         ...
         RETURNING id`,
            ...
            await c.query(
                `UPDATE broker_state SET active_job_id = NULL WHERE active_job_id IS NOT NULL`,
            );
...
function requireRef(value: string | null | undefined, label: string): string {
    if (!value)
        throw new Error(`${label} job is missing its deployment reference`);
    return value;
}
