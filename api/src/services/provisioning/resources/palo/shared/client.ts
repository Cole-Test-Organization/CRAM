...
export class PanosApiClient {
  ...
  async generateApiKey(username: string, password: string): Promise<string> {
    ...
    if (!key) throw new PanosApiError("PAN-OS keygen response did not include an API key", body);
    this.apiKey = key;
    return key;
  }
  ...
  async setSerial(serial: string): Promise<void> {
    await this.op(`<set><serial-number>${xmlEscape(serial)}</serial-number></set>`, {
  ...
  async setLicenseDeactivationApiKey(apiKey: string): Promise<void> {
    // Installs the CSP Licensing API Key (Assets -> Licensing API on the support
    // portal). Auto-mode deactivation requires this key so the firewall can
    // authenticate to the licensing server on its own behalf. Distinct from the
    // firewall admin API key used for every other call.
    await this.op(
  ...
  async deactivateVmSeriesLicense(timeoutMs = 300_000): Promise<void> {
    // Auto mode: the firewall contacts the Palo Alto licensing server directly
    // (via its management interface) and releases the Flex/VM-Series credits so
    // they are not orphaned when the VM is destroyed. The PAN-OS API element is
    // <VM-Capacity> (the CLI keyword is "VM-Capacity"), not <VM-Series>. PAN-OS
    // returns either an immediate result or a job id to poll.
    ...
    if (jobId) {
      ...
      await this.waitForJob(jobId, timeoutMs, `license deactivation job ${jobId}`);
      this.log(`PAN-OS license deactivation finished (job ${jobId})`);
    } else {
      this.log("PAN-OS license deactivation completed (no job id returned)");
    }
  }
  ...
  async generateVmAuthKey(lifetimeHours: number): Promise<VmAuthKeyResult> {
    ...
    const authKey =
      ...
      textMatch(body, /(?:auth(?:entication)?\s*key|vm-auth-key)\s*[:=]\s*([^\s<]+)/i);
    if (!authKey) throw new PanosApiError("VM auth key response did not include an auth key", body);
  ...
  async showConnectedDevices(): Promise<ConnectedDevice[]> {
    const body = await this.op("<show><devices><connected></connected></devices></show>");
  ...
  async ensureTemplateStack(templateStackName: string, templateName: string): Promise<void> {
    await this.configSet(
  ...
  async addDeviceToTemplateStack(templateStackName: string, serial: string): Promise<void> {
    await this.configSet(
  ...
  async addDeviceToDeviceGroup(deviceGroupName: string, serial: string, vsys = "vsys1"): Promise<void> {
    await this.configSet(
  ...
  async commitAllTemplateStack(
    ...
    if (!serials.length) throw new Error("Template stack push needs at least one firewall serial");
    const members = serials.map((serial) => `<member>${xmlEscape(serial)}</member>`).join("");
  ...
  async commitAllDeviceGroup(
    ...
    if (!devices.length) throw new Error("Device group push needs at least one firewall serial");
  ...
  async waitForJob(jobId: string, timeoutMs = 600_000, label?: string): Promise<void> {
    const what = label ?? `job ${jobId}`;
    const deadline = Date.now() + timeoutMs;
    let lastProgress: string | null = null;
    while (Date.now() < deadline) {
      const body = await this.op(`<show><jobs><id>${xmlEscape(jobId)}</id></jobs></show>`);
      ...
      if (status === "FIN") {
        ...
        throw new PanosApiError(`PAN-OS ${what} finished with result ${result}`, body);
      }
      ...
      if (progress && progress !== lastProgress) {
        this.log(`PAN-OS ${what} progress ${progress}% (status ${status ?? "ACT"})`);
  ...
  async op(cmd: string, options: { ignoreDisconnect?: boolean } = {}): Promise<string> {
  ...
  private async commitAll(cmd: string, timeoutMs: number, label = "commit-all"): Promise<void> {
    const body = await this.request({
      type: "commit",
      action: "all",
      cmd,
    ...
    const jobId = xmlText(body, "job");
    if (!jobId) throw new PanosApiError("PAN-OS commit-all response did not include a job id", body);
  ...
  private requireApiKey(): string {
    if (!this.apiKey) throw new Error("PAN-OS API key has not been generated");
    return this.apiKey;
  }
  ...
  private async request(
    ...
    return await new Promise<string>((resolve, reject) => {
      ...
      request.on("timeout", () => {
        request.destroy(new Error(`PAN-OS API request to ${this.options.host} timed out`));
      });
      request.on("error", (error: NodeJS.ErrnoException) => {
        if (options.ignoreDisconnect && ["ECONNRESET", "EPIPE", "ETIMEDOUT"].includes(error.code ?? "")) {
          resolve("");
          return;
        }
        reject(error);
...
function panoramaBaseXpath(child: "device-group" | "template" | "template-stack"): string {
  return `/config/devices/entry[@name='localhost.localdomain']/${child}`;
}

function xpathLiteral(value: string): string {
  if (value.includes("'")) {
    throw new Error(`PAN-OS xpath values cannot include single quotes: ${value}`);
  }
  return value;
}
...
function assertPanosSuccess(body: string): void {
  ...
  const message = xmlText(body, "msg") ?? xmlText(body, "line") ?? stripXml(body).trim();
...
function firstLeafXmlText(body: string, tagName: string): string | null {
  const escaped = tagName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const matches = [...body.matchAll(new RegExp(`<${escaped}(?:\\s+[^>]*)?>([^<>]*)<\\/${escaped}>`, "gi"))];
