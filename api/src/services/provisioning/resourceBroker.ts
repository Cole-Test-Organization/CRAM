...
export class ResourceBroker {
  ...
  constructor(options: ResourceBrokerOptions = {}) {
    this.store = options.store ?? new FileStateRepository();
  ...
  async loadProviderProfile(profileName: string): Promise<ProviderConfig> {
    ...
    if (!profile) throw new Error(`provider profile ${profileName} not found`);
    return profile;
  }
  ...
  async loadAppProfile<TProfile = unknown>(
    ...
    const profile = await this.config.getAppProfile<TProfile>(group, profileName);
    if (profile == null) throw new Error(`app profile ${group}/${profileName} not found`);
    return profile;
  }
  ...
  async loadConfigProfile<TProfile = unknown>(
    ...
    const profile = await this.config.getConfigProfile<TProfile>(group, profileName);
    if (profile == null) throw new Error(`config profile ${group}/${profileName} not found`);
    return profile;
  }
  ...
  async loadTerraformResourceProfile(
    ...
    if (!profile) throw new Error(`terraform resource profile ${profileName} not found`);
    return profile;
  }
  ...
  private async applyProviderProfile(
    ...
    if (typeof deployment.providerProfile !== "string") {
      throw new Error(`Invalid config ${configRef}: providerProfile must be a string`);
    }

    const profile = await this.loadProviderProfile(deployment.providerProfile);
    return {
      ...deployment,
      provider: {
        ...profile,
  ...
  private mapYamlGroupPath(filePath: string): string | null {
    ...
    if (inYamlGroup(normalized, "database/deployments") || inYamlGroup(normalized, "deployments")) {
    ...
    if (
      ...
      return path.join(this.rootDir, "database", "provider-profiles", basename);
    }
    if (
      inYamlGroup(normalized, "database/resource-profiles") ||
      inYamlGroup(normalized, "profiles/resources")
    ) {
      return path.join(this.rootDir, "database", "resource-profiles", basename);
    }
  ...
  async up(
    ...
    const initialState = adapter.initialState?.(
      deployment,
      resource,
      provider,
      configRef,
    ) ?? {};
    const record = await this.store.upsertResource({
      id: "",
    ...
    log(`Starting ${resource.kind} provisioning for ${resource.hostname} with provider ${provider.type}`);
    await this.store.patchResource(record.id, { lifecycleStatus: "terraform_applying" });
    const upResult = await adapter.up(
      this.resourceAdapterContext(configRef, deployment, resource, provider, options.params),
      record,
      log,
    );

    const next = await this.store.patchResource(record.id, {
  ...
  async down(
    ...
    const provider = getProviderAdapter(record.provider ?? deployment.provider.type);
    const adapter = this.resourceAdapters.resolve(selected.kind);
    await this.store.patchResource(record.id, { lifecycleStatus: "destroy_requested" });
    log(`Starting teardown for ${record.hostname}`);

    await this.store.patchResource(record.id, { lifecycleStatus: "terraform_destroying" });
    await adapter.down(
      this.resourceAdapterContext(record.configPath, deployment, selected, provider, options.params),
      record,
      log,
    );
    const next = await this.store.patchResource(record.id, { lifecycleStatus: "destroyed" });
  ...
  async runAction(
    ...
    const configScopedTargetRecord =
      ...
      this.resolveYamlPath(targetRecord.configPath) === this.resolveYamlPath(configRef)
        ? targetRecord
        : null;
    ...
    if (!adapter.runAction) {
      throw new Error(`Resource kind ${resource.kind} does not support resource actions`);
    }
  ...
  async deploy(
    ...
    for (const step of deployment.steps) {
      const stepParams = mergeRunParams(options.params, step.params);
      if (!shouldRunDeploymentStep(step, stepParams)) {
        log(`Skipping step ${step.name}: condition not met`);
        continue;
      }

      log(`Starting step ${step.name}: ${step.description ?? step.action}`);
      const targets = step.targets ?? [];
      if (!targets.length) throw new Error(`Step ${step.name} needs at least one target`);
      ...
      } else if (step.action === "down" || step.action === "destroy") {
        for (const target of [...targets].reverse()) {
          await this.down(target, log, {
            skipActiveJobCheck: true,
            params: stepParams,
          });
        }
  ...
  async deprovision(
    ...
    for (const step of [...(deployment.steps ?? [])].reverse()) {
      if (!isProvisionStepAction(step.action)) {
        log(`Skipping step ${step.name}: ${step.action} has no automatic teardown`);
        continue;
      }

      const stepParams = mergeRunParams(options.params, step.params);
      const shouldRunStep = shouldRunDeploymentStep(step, stepParams);
      provisionStepCount += 1;
      log(
        `Starting teardown for step ${step.name}: ${step.description ?? step.action}` +
          (shouldRunStep ? "" : " (condition not met; checking for existing resources only)"),
      );
      const targets = step.targets ?? [];
      if (!targets.length) throw new Error(`Step ${step.name} needs at least one target`);
        ...
        if (!record) {
          log(
            `Skipping ${resource.hostname}: ` +
              (shouldRunStep ? "no broker state record exists" : "condition not met and no broker state record exists"),
          );
          continue;
        }
  ...
  async listResources(): Promise<ResourceRecord[]> {
    return await this.store.listResources();
  }

  async getResource(target: string): Promise<ResourceRecord | null> {
    return await this.store.getResource(target);
  }
  ...
  async refreshResourcePowerState(
    ...
    if (!provider.getResourcePowerState) {
      throw new Error(`Provider ${provider.type} does not support resource power state checks`);
    }
    if (provider.supportsPowerControl && !provider.supportsPowerControl(context, record)) {
  ...
  subscribeEvents(listener: BrokerEventListener): () => void {
    return this.store.events.subscribe(listener);
  }
  ...
  async powerResources(
    ...
    if (!options.skipActiveJobCheck) await this.ensureNoActiveJob();
    const resources = selectResources(await this.store.listResources(), selector);
    if (!resources.length) {
      throw new Error("No resources matched the requested power action selector");
    }
    ...
    for (const record of resources) {
      try {
        ...
        if (!support.supported) {
          const message = support.reason ?? `${record.hostname} is not power-controllable`;
        ...
        const resource = await this.runResourcePowerAction(action, record, log);
        results.push({
          action,
          resource,
          status: "succeeded",
  ...
  async status(hostname?: string): Promise<unknown> {
    ...
    return await this.store.getResource(hostname);
  }
  ...
  private async requireExistingResource(target: string): Promise<ResourceRecord> {
    const record = await this.store.getResource(target);
    if (!record) throw new Error(`No resource named ${target} exists in broker state`);
    return record;
  }
  ...
  private async requireResource(target: string): Promise<ResourceRecord> {
    ...
    if (!record) throw new Error(`No resource named ${target} exists in broker state`);
    if (record.lifecycleStatus === "destroyed") {
      throw new Error(`${record.hostname} has been destroyed and cannot be power-controlled`);
    }
    return record;
  }
  ...
  private async runResourcePowerAction(
    ...
    const { context, provider } = await this.resolveResourcePowerContext(record);
    const method = action === "start" ? provider.startResource : provider.stopResource;
    if (!method) {
      throw new Error(`Provider ${provider.type} does not support resource ${action}`);
    }
    if (provider.supportsPowerControl && !provider.supportsPowerControl(context, record)) {
      throw new Error(`${record.hostname} is not power-controllable by provider ${provider.type}`);
    }

    const pendingState: ResourcePowerState = action === "start" ? "pending" : "stopping";
    const pending = await this.store.patchResource(record.id, {
  ...
  private async getPowerSupport(
    ...
    if (!record.providerResourceId) {
      return { supported: false, reason: "no provider resource id is recorded" };
    }
  ...
  private async ensureNoActiveJob(): Promise<void> {
    ...
    if (state.activeJobId) {
      throw new Error(`Another job is already running: ${state.activeJobId}`);
    }
  }
}
...
function selectResources(
  ...
  const configPath = selector.configPath ? resolveProjectFilePath(selector.configPath) : null;

  return resources.filter((resource) => {
    if (targets.size > 0 && !resourceMatchesTarget(resource, targets)) return false;
    if (selector.deploymentId && resource.deploymentId !== selector.deploymentId) return false;
    if (configPath && resolveProjectFilePath(resource.configPath) !== configPath) return false;
    if (includeKinds.size > 0 && (!resource.kind || !includeKinds.has(resource.kind))) return false;
    if (resource.kind && excludeKinds.has(resource.kind)) return false;
    return true;
  });
}

function resourceMatchesTarget(resource: ResourceRecord, targets: Set<string>): boolean {
  return (
...
function shouldRunDeploymentStep(
  step: DeploymentStepConfig,
  params: Record<string, unknown> | undefined,
): boolean {
  if (step.enabled === false) return false;
  if (!step.when) return true;
  const expected = step.when.equals ?? true;
  return params?.[step.when.param] === expected;
}

function findResource(deployment: DeploymentConfig, target?: string): ResourceConfig {
  const resource = target
    ? deployment.resources.find(
        (candidate) => candidate.hostname === target || candidate.name === target,
      )
    : deployment.resources[0];
  if (!resource) {
    const suffix = target ? ` named ${target}` : "";
    throw new Error(`Deployment ${deployment.name} has no resource${suffix}`);
  }
  return resource;
}
...
function validateDeploymentConfig(
  ...
  if (!deployment.provider?.type) throw new Error(`${prefix} provider.type is required`);
  if (!deployment.resources?.length) throw new Error(`${prefix} resources must not be empty`);
  ...
  for (const step of deployment.steps ?? []) {
    ...
    if (step.when !== undefined) {
      if (!isRecord(step.when)) throw new Error(`${stepPrefix} when must be an object`);
      ...
      if (
        ...
        throw new Error(`${stepPrefix} when.equals must be a string, number, boolean, or null`);
      }
    }
  }
  ...
  for (const resource of deployment.resources) {
    ...
    if (!resource.kind) throw new Error(`${resourcePrefix} kind is required`);
    if (!resource.hostname) throw new Error(`${resourcePrefix} hostname is required`);
    validateResourcePlacement(deployment, resource.placement, resourcePrefix);
  }
}
...
function validateNetworkPlacement(
  ...
  if (!isRecord(network)) throw new Error(`${prefix} placement.network must be an object`);
  ...
  if (
    ...
    throw new Error(`${prefix} placement.network.mode must be managed or existing`);
  }

  if (network.interfaces !== undefined && network.interfaces !== null) {
    if (!isRecord(network.interfaces)) {
      throw new Error(`${prefix} placement.network.interfaces must be an object`);
    }
    for (const [name, value] of Object.entries(network.interfaces)) {
      if (!isRecord(value)) {
        throw new Error(`${prefix} placement.network.interfaces.${name} must be an object`);
      }
    }
  }

  if (network.nextHop !== undefined && network.nextHop !== null) {
    if (!isRecord(network.nextHop)) throw new Error(`${prefix} placement.network.nextHop must be an object`);
    if (
      network.nextHop.type !== undefined &&
      network.nextHop.type !== null &&
      typeof network.nextHop.type !== "string"
    ) {
      throw new Error(`${prefix} placement.network.nextHop.type must be a string`);
    }
  }
...
function hasExistingNetworkReference(network: NonNullable<ResourcePlacementConfig["network"]>): boolean {
  return (
    hasPlacementValue(network.vpcId) ||
    hasPlacementValue(network.subnetId) ||
    hasPlacementValue(network.routeTableId) ||
    Boolean(network.interfaces && Object.keys(network.interfaces).length > 0)
  );
}
