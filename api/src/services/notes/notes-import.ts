...
// Normalized metadata for one note, as produced by normalizeExtraction.
...
// In-memory record for one import job. `_files` (the staged source files) is
// held off the public job view via Object.defineProperty so polls stay small.
interface NotesImportJob {
  jobId: string;
  userId: number;
...
interface NotesImportDeps {
  meetingsService?: any;
  accountsService?: any;
  agentSettingsService?: any;
  extractor?: ((file: NoteFile, opts: { baseUrl: string | null; model: string }) => unknown) | null;
}
...
export function normalizeExtraction(obj: any): ExtractionRecord {
...
export class NotesImportService {
  ...
  accountsService: any;
  agentSettingsService: any;
  extractor: NotesImportDeps['extractor'];
  jobs: Map<string, NotesImportJob>;
  queue: string[];
  running: boolean;
  ...
  constructor({ meetingsService, accountsService, agentSettingsService, extractor }: NotesImportDeps = {}) {
  ...
  enqueue(userId: number, { files }: { files?: any[] } = {}) {
    ...
    try {
      while (this.queue.length > 0) {
        ...
        try {
          ...
          logger.error({ event: 'notes_import.run_crashed', err: (err as Error).message, stack: (err as Error).stack, jobId }, 'notes-import run crashed');
          const j = this.jobs.get(jobId);
  ...
  async _run(jobId: string) {
    ...
    if (this.agentSettingsService) {
      try {
        ...
        logger.warn({ event: 'notes_import.settings_lookup_failed', err: (err as Error).message }, 'failed to resolve agent settings — falling back to env');
      }
    }
...
Return ONLY the JSON object specified in the system prompt.`;
    ...
    for (let attempt = 1; attempt <= LLM_MAX_ATTEMPTS; attempt++) {
      try {
        ...
        if (attempt < LLM_MAX_ATTEMPTS) {
          ...
          logger.warn({ event: 'notes_import.llm_retry', attempt, maxAttempts: LLM_MAX_ATTEMPTS, waitMs, err: (err as Error).message }, 'local LLM call failed — retrying');
          await sleep(waitMs);
        }
      }
    }
  ...
  async _writeOne(userId: number, file: NoteFile, ex: ExtractionRecord) {
    ...
    let decision: { kind: string; reason?: string; account?: any; matched_by?: any; match_score?: any; candidates?: any };
    if (!hasHint) {
