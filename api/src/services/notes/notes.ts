...
function pickTarget(target: { account_id?: number | null; contact_id?: number | null; opportunity_id?: number | null }) {
  ...
  const present = TARGETS
    .map((k) => [k, values[k]] as [string, number | null | undefined])
    .filter(([, v]) => v != null);
  if (present.length !== 1) {
...
export class NotesService {
  async getAll(userId: number, { account_id, contact_id, opportunity_id, limit, offset = 0 }: { account_id?: number | null; contact_id?: number | null; opportunity_id?: number | null; limit?: number | null; offset?: number } = {}) {
  ...
  async create(userId: number, { account_id, contact_id, opportunity_id, body }: { account_id?: number | null; contact_id?: number | null; opportunity_id?: number | null; body?: string }) {
    const { key, value } = pickTarget({ account_id, contact_id, opportunity_id });
    return withUser(userId, async (client) => {
  ...
  async patch(userId: number, id: number, { body }: { body?: string }) {
    return withUser(userId, async (client) => {
