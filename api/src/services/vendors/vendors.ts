...
export class VendorsService {
  async getAll({ search, include_deleted = false, needs_review, limit, offset = 0 }: { search?: string; include_deleted?: boolean; needs_review?: boolean; limit?: number | null; offset?: number } = {}) {
  ...
  async findOrCreate({ name, slug, website, notes }: { name?: string; slug?: string; website?: string | null; notes?: string | null }) {
    if (!name || !name.trim()) {
  ...
  async patch(id: number, data: { name?: string; slug?: string; website?: string | null; notes?: string | null; needs_review?: boolean }) {
    const client = await getPool().connect();
    try {
