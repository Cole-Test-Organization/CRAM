...
import { normalizeDomain, parseDomainList } from '../_shared/_domain.js';
...
export class InternalDomainsService {
  ...
  async remove(userId: number, domain: string) {
    const normalized = normalizeDomain(domain);
    if (!normalized) {
    ...
    return withUser(userId, async (client) => {
      ...
      return { domain: normalized, deleted: (result.rowCount ?? 0) > 0 };
    });
  }
}
