...
interface OutreachJob {
  jobId: string;
  type: string;
  params: { name: string; company?: string; title?: string; deep: boolean; limit?: number; linkedin: boolean };
...
export class OutreachService {
  ...
  enqueue({ type, name, company, title, deep, limit, linkedin = true }: { type: string; name: string; company?: string; title?: string; deep?: boolean; limit?: number; linkedin?: boolean }) {
  ...
  async _execute(job: OutreachJob) {
  ...
  _publicView(job: OutreachJob, { includePosition = false }: { includePosition?: boolean } = {}) {
    const view: OutreachJob = {
      jobId: job.jobId,
      type: job.type,
      params: job.params,
      status: job.status,
