...
export class AccountDetailsService {
  ...
  async upsert(userId: number, accountId: number, data: Record<string, any>) {
    return withUser(userId, async (client) => {
  ...
  async _expandVendorProducts(client: PoolClient, row: any) {
    ...
    for (const col of VENDOR_ARRAY_COLS) {
      const key = col.replace(/_ids$/, '_products');
      expanded[key] = (row[col] || []).map((id: number) => byId.get(Number(id))).filter(Boolean);
    }
    return expanded;
  }
}
