...
import { deriveFilename, slugify } from '../_shared/_slug.js';
import { jsonb } from '../_shared/_json.js';
import { badRequest } from '../../lib/http-error.js';

// Minimal shapes for the optionally-injected sibling services. The importer only
// touches their dedupe/insert helpers; the full services are richer but these are
// the only members this file calls. Both are optional — when absent the code
...
export class ImportExportService {
  ...
  constructor({ contactsService, accountsService }: { contactsService?: ContactsServiceLike; accountsService?: AccountsServiceLike } = {}) {
  ...
  async exportAccounts(userId: number, slugs: string[]) {
  ...
  async _exportSingleAccount(client: PoolClient, slug: string) {
  ...
  async _exportDetails(client: PoolClient, accountId: number) {
    ...
    for (const col of VENDOR_ARRAY_COLS) {
      out.vendor_products[col] = (row[col] || [])
        .map((id: any) => refsById.get(Number(id)))
        .filter(Boolean)
        .map((r: any) => ({
  ...
  async _exportContactsForAccount(client: PoolClient, accountId: number) {
  ...
  async _exportMeetings(client: PoolClient, accountId: number) {
  ...
  async _exportOpportunities(client: PoolClient, accountId: number) {
  ...
  async _exportPartnerShells(client: PoolClient, accountId: number) {
  ...
  async importBundle(userId: number, bundle: any) {
  ...
  async _importOneAccount(userId: number, acctJson: any) {
  ...
  async _upsertAccount(client: PoolClient, j: any, tally: any, updated: any) {
  ...
  async _upsertDetails(client: PoolClient, accountId: number, d: any) {
  ...
  async _resolveVendorProduct(client: PoolClient, ref: any, sourceCol: string) {
    if (!ref) return null;
  ...
  async _upsertPartners(client: PoolClient, accountId: number, partners: any[], tally: any) {
    const out = [];
    for (const p of partners) {
      if (!p?.slug) continue;
  ...
  async _findOrCreatePartnerAccount(client: PoolClient, p: any, tally: any) {
    const existing = this.accountsService
  ...
  async _upsertContacts(client: PoolClient, accountId: number, contacts: any[], tally: any, updated: any) {
    const map = new Map();
  ...
  async _buildAccountContactLookup(client: PoolClient, accountId: number) {
    const rows = (await client.query(
  ...
  async _upsertMeetings(client: PoolClient, accountId: number, meetings: any[], lookup: { byEmail: Map<string, any>; byName: Map<string, any> }, tally: any, updated: any) {
    for (const m of meetings) {
  ...
  async _upsertOpportunities(client: PoolClient, accountId: number, opps: any[], tally: any, updated: any) {
    for (const o of opps) {
      if (!o?.name) continue;
  ...
  async _upsertProduct(client: PoolClient, p: any, tally: any) {
    let categoryId = null;
    if (p.category_name) {
