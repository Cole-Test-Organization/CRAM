...
export class ThreadsService {
  ...
  async getAllForAccount(userId: number, accountId: number | string, { include_closed = false }: { include_closed?: boolean } = {}) {
  ...
  async create(userId: number, { account_id, title, description, contact_ids }: { account_id?: number | string | null; title?: string; description?: string | null; contact_ids?: Array<number | string> } = {}) {
    ...
    return withUser(userId, async (client) => {
      ...
      try {
        ...
        if ((err as { code?: string }).code === '23503') throw notFound(`Account ${account_id} not found.`);
        throw err;
      }
  ...
  async patch(userId: number, id: number, data: { title?: string; description?: string | null; closed?: boolean } = {}) {
  ...
  async createTask(userId: number, threadId: number | string, { title, description, assignee_contact_id, due_date }: { title?: string; description?: string | null; assignee_contact_id?: number | null; due_date?: string | null } = {}) {
  ...
  async patchTask(userId: number, taskId: number, data: { title?: string; description?: string | null; assignee_contact_id?: number | null; due_date?: string | null; completed?: boolean } = {}) {
    if (data.title !== undefined && !String(data.title).trim()) {
  ...
  async linkContact(userId: number, threadId: number | string, contactId: number | string) {
    return withUser(userId, async (client) => {
      const thread = (await client.query('SELECT id FROM threads WHERE id = $1', [threadId])).rows[0];
      if (!thread) throw notFound(`Thread ${threadId} not found.`);
      ...
      return this._fetch(client, Number(threadId));
    });
  }

  async unlinkContact(userId: number, threadId: number | string, contactId: number | string) {
    return withUser(userId, async (client) => {
      const thread = (await client.query('SELECT id FROM threads WHERE id = $1', [threadId])).rows[0];
      if (!thread) throw notFound(`Thread ${threadId} not found.`);
  ...
  async _enrich(client: PoolClient, threads: any[]) {
  ...
  async _assertContactVisible(client: PoolClient, contactId: number | string) {
  ...
  async _taskWithAssignee(client: PoolClient, task: any) {
    if (!task) return task;
