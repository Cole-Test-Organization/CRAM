...
export class ContactEnrichmentService {
  constructor({ outreachService, contactsService, agentSettingsService, getDefaultUserId } = {}) {
  ...
  enqueue(userId, { contactId, name, accountName }) {
  ...
  listJobsForContacts(contactIds) {
  ...
  async _run(jobId) {
    ...
    try {
      ...
      job.status = 'failed';
      job.error = `outreach enqueue failed: ${err.message || String(err)}`;
    ...
    if (this.agentSettingsService) {
      try {
        ...
        logger.warn({ event: 'enrichment.settings_lookup_failed', err: err.message }, 'failed to resolve agent settings — falling back to env');
      }
    }
    let patch;
    try {
      ...
      job.status = 'failed';
      job.error = `formatter error: ${err.message || String(err)}`;
      job.completedAt = new Date().toISOString();
      return;
    }
    ...
    try {
      ...
      job.status = 'failed';
      job.error = `contact patch failed: ${err.message || String(err)}`;
      job.completedAt = new Date().toISOString();
      return;
    }
