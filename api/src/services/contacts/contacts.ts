...
import { fillBlanks } from '../_shared/_enrich.js';
import { normalizeDomain, suggestAccountName } from '../_shared/_domain.js';
import { parseEmailList } from '../_shared/_email.js';
import { slugify } from '../_shared/_slug.js';
import type { AccountsService } from '../accounts/accounts.js';
import type { InternalDomainsService } from '../internal-domains/internal-domains.js';
import type { ContactEnrichmentService } from './contact-enrichment.js';
...
function normalizeKind(kind: string | null | undefined): string | undefined {
...
export class ContactsService {
  ...
  contactEnrichmentService: ContactEnrichmentService | null;
  ...
  constructor({ accountsService = null, internalDomainsService = null, contactEnrichmentService = null }: { accountsService?: AccountsService | null; internalDomainsService?: InternalDomainsService | null; contactEnrichmentService?: ContactEnrichmentService | null } = {}) {
    this.accountsService = accountsService;
    this.internalDomainsService = internalDomainsService;
    this.contactEnrichmentService = contactEnrichmentService;
  }

  async getAll(userId: number, { company, search, kind, city, country, limit, offset = 0 }: { company?: string | null; search?: string | null; kind?: string | null; city?: string | null; country?: string | null; limit?: number | null; offset?: number } = {}) {
  ...
  async getByEmailBrief(userId: number, email: string) {
    if (!email || typeof email !== 'string') return null;
    const normalized = email.trim().toLowerCase();
    if (!normalized) return null;
    return withUser(userId, async (client) => {
  ...
  async getAttendeeOptions(userId: number, { mode, accountId }: { mode?: string | null; accountId?: number | null } = {}) {
    return withUser(userId, async (client) => {
  ...
  async _findExisting(client: PoolClient, data: ContactData) {
    if (!data) return null;
  ...
  async findExisting(userId: number, data: ContactData) {
    return withUser(userId, async (client) => {
  ...
  async _fetchWithAccounts(client: PoolClient, id: number) {
  ...
  async _linkAndFetch(client: PoolClient, contactId: number, accountId?: number | null) {
    if (accountId) {
      await client.query(
  ...
  async linkAccount(userId: number, contactId: number, accountId: number) {
    return withUser(userId, async (client) => {
      await client.query(
        'INSERT INTO account_contacts (account_id, contact_id) VALUES ($1, $2) ON CONFLICT DO NOTHING',
        [accountId, contactId]
  ...
  async unlinkAccount(userId: number, contactId: number, accountId: number) {
    return withUser(userId, async (client) => {
      await client.query(
  ...
  async reassignAccount(userId: number, contactId: number, fromAccountId: number | null | undefined, toAccountId: number) {
    if (!toAccountId) {
  ...
  //     primary_domain }
  async resolveEmails(userId: number, emails: string | string[]) {
    ...
    const attendees: Array<{ email: string; domain: string | null; name_guess: string | null; kind: string; contact: any; account_match?: any }> = [];
  ...
  async importFromEmails(userId: number, payload: {
    account?: { mode?: string | null; account_id?: number | null; name?: string | null; domain?: string | null } | null;
    contacts?: Array<{ mode?: string | null; contact_id?: number | null; link_to_account?: boolean | null; full_name?: string | null; email?: string | null; kind?: string | null; research?: boolean | null }>;
  }) {
    if (!this.accountsService) {
