...
export function parseEmailList(text: unknown): ParsedEmail[] {
  if (!text) return [];
  const seen = new Map<string, ParsedEmail>();
  const EMAIL_BODY = '[A-Z0-9._%+\\-]+@[A-Z0-9.\\-]+\\.[A-Z]{2,}';

  function add(email: string, displayName: string): void {
  ...
  const remainder = String(text).replace(bracketRe, (_full: string, q1: string, q2: string, plain: string, email: string) => {
