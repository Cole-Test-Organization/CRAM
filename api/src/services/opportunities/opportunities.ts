...
function normalizeReasonList(value: unknown, field: string) {
...
export class OpportunitiesService {
  async getAll(userId: number, { account_id, stage, sort = 'created_at', order = 'desc', limit = 100, offset = 0 }: { account_id?: number; stage?: string; sort?: string; order?: string; limit?: number; offset?: number } = {}) {
  ...
  async patch(userId: number, id: number, data: Record<string, any>) {
    return withUser(userId, async (client) => {
  ...
  async linkProduct(userId: number, opportunityId: number, productId: number) {
    return withUser(userId, async (client) => {
  ...
  async unlinkProduct(userId: number, opportunityId: number, productId: number) {
    return withUser(userId, async (client) => {
      const opp = (await client.query('SELECT id FROM opportunities WHERE id = $1', [opportunityId])).rows[0];
      if (!opp) return null;
      await client.query(
  ...
  async _assertNotPartnerAccount(client: PoolClient, accountId: number) {
  ...
  async _setProducts(client: PoolClient, opportunityId: number, productIds: any[]) {
  ...
  async _fetchWithProducts(client: PoolClient, id: number) {
