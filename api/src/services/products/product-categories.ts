...
export class ProductCategoriesService {
  async getAll(userId: number, { limit, offset = 0 }: { limit?: number | null; offset?: number } = {}) {
  ...
  async create(userId: number, { name }: { name?: string }) {
    if (!name || !name.trim()) {
    ...
    return withUser(userId, async (client) => {
      const inserted = await client.query(
        `INSERT INTO product_categories (user_id, name)
         ...
         RETURNING id`,
        [name.trim()]
      ).catch(rethrowUniqueViolation(`Category "${name.trim()}" already exists`));
      return this._fetch(client, inserted.rows[0].id);
    });
  }

  async patch(userId: number, id: number, { name }: { name?: string }) {
    return withUser(userId, async (client) => {
      ...
      await client.query(
        ...
        [id, newName]
      ).catch(rethrowUniqueViolation(`Category "${newName}" already exists`));
